"""Loaders for the inputevents table. Inputevents covers all fluids that are administered to patients via IV or other routes.
Inputevents loaders data register naming convention: '<ITEMID>_<AMOUNTUOM>'
"""
...
def load_inputevents(
    ...
    """
    inputevents_file_path = DATA_PATH / "mimic-iii-clinical-database-1.4" / "INPUTEVENTS_MV.csv.gz"
...
def load_fentanyl(
    ...
    df = _drop_rows_with_too_small_value_frequency(df=df, value_col_name="AMOUNTUOM", threshold=0.1)
...
def _calc_amount_to_bodyweight_ratio(df: pd.DataFrame) -> pd.DataFrame:
    """Calculates the amount to bodyweight ratio for each row in the inputevents table."""
