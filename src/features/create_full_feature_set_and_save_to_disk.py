...
def main(save_to_disk: bool = False, min_set_for_debug: bool = False):
    ...
    flattened_df = generate_flattened_features(save_to_disk=save_to_disk, min_set_for_debug=min_set_for_debug,)
    ...
    admission_level_vectors_df = aggregate_co_vectors(co_df=co_df, binary_feature_df=binary_feature_df,)
    ...
    if save_to_disk:
        if project_info.dataset_format == "parquet":
            ...
            admission_level_vectors_df.to_csv(project_info.feature_set_path / "admission_level_vectors_df.csv")
