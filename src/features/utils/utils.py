"""Misc. functions for feature generation."""
...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    All values in numeric columns that are in the top or bottom 1% of the distribution are set to 0.
    ...
    non_text_feature_columns = [column for column in numeric_cols.columns if not column.startswith('pred_text_')]
    ...
    for column in numeric_cols.columns:
        ...
        bottom_quantile = numeric_cols[column].loc[numeric_cols[column] != 0].quantile(0.01)
        top_quantile = numeric_cols[column].quantile(0.99)

        is_outlier = (numeric_cols[column] > top_quantile) | (numeric_cols[column] < bottom_quantile)
