...
def main(   
    ...
    if read_flattened_features_from_disk:
        ...
        flattened_df = pd.read_csv(
            project_info.project_path / "data" / "feature_sets" / flattened_feature_set_path / "flattened_features.csv",
        )   
    else:
    ...
    print("Aggregating co-occurrence counts to admission level vectors...", curr_timestamp)
    ...
    train_binary_feature_df = binary_feature_df[binary_feature_df['patient_id'].isin(train_patients)]
    test_binary_feature_df = binary_feature_df[binary_feature_df['patient_id'].isin(test_patients)]
    ...
    train_admission_level_vectors_df = admission_level_vectors_df[admission_level_vectors_df['patient_id'].isin(train_patients)]
    test_admission_level_vectors_df = admission_level_vectors_df[admission_level_vectors_df['patient_id'].isin(test_patients)]

    # print the shapes of the train and test sets for the co_df
    print(f'Train set shape (admission_level_vectors_df): {train_admission_level_vectors_df.shape}')
    print(f'Test set shape (admission_level_vectors_df): {test_admission_level_vectors_df.shape}')
...
if __name__ == "__main__":
    main(flattened_feature_set_path="multimodal_rep_learning_ehr_features_2023_05_09_15_09",
